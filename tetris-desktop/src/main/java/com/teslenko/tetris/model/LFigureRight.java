...
public class LFigureRight extends Figure {
	private List<Dot> dots;
	private LFigurePositions position;

	public LFigureRight(String color, LFigurePositions position) {
		this(color, 0, position);
	}

	public LFigureRight(String color, int initX, LFigurePositions position) {
		super(color);
	...
	public List<Dot> getDotsAfterRotation() {
		...
		if(position == LFigurePositions.HOR_LEFT) {
			...
			Dot init = dots.get(0);
			buildVertLow(init.getX(), init.getY(), dotsAfterRotation);
		} else if(LFigurePositions.VERT_LOW == position) {
			Dot init = dots.get(0);
			buildHorLeft(init.getX()-1, init.getY(), dotsAfterRotation);
		}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		LFigureRight other = (LFigureRight) obj;
		if (dots == null) {
			if (other.dots != null)
				return false;
